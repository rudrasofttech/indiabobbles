...
namespace IndiaBobbles.Models
{
    ...
    public class VisitManager
    {
        ...
        {
            HttpContext.Current.Trace.Write(xml);
            ...
            try
            {
                ...
                if (doc.GetElementsByTagName("a").Count > 0)
                {
                    ...
                    {
                        if (currentView.PageUrlRaw.ToLower() != doc.GetElementsByTagName("v")[0].InnerText.ToLower())
                        {
                            ViewInfo vi = new ViewInfo();

                            vi.PageUrlRaw = doc.GetElementsByTagName("v")[0].InnerText.ToLower();
                            vi.PageUrlPath = string.Empty;
                            vi.PageUrlData = string.Empty;
