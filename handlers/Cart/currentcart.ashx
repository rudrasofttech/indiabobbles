    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            if (context.Request["action"] != null)
            {
                ...
                {
                    om.UpdateOrderPayment(int.Parse(CookieWorker.GetCookie(CookieWorker.OrderIdKey, "cartid")), "COD", DateTime.Now, "Cash on Delivery");
                    om.UpdateOrderStatus(int.Parse(CookieWorker.GetCookie(CookieWorker.OrderIdKey, "cartid")), OrderStatusType.CODPaid, "Payment will be done on delivery.");
                    context.Response.ContentType = "text/json";
                    context.Response.Write(serializer.Serialize(new { Cart = GetCartDynamicObject(om.GetCart()), Success = true }));
                }
                ...
                else if (action == "removeitem")
                {
                    ...
                    if (error.Count == 0)
                    {
                        om.RemoveItem(itemid, int.Parse(CookieWorker.GetCookie(CookieWorker.OrderIdKey, "cartid")));
                        context.Response.Write(serializer.Serialize(new { Cart = GetCartDynamicObject(om.GetCart()), Success = true }));
                    }
                    else
                    {
                ...
                else if (action == "updateitem")
                {
                    ...
                    if (error.Count == 0)
                    {
                        om.UpdateItem(itemid, quantity, int.Parse(CookieWorker.GetCookie(CookieWorker.OrderIdKey, "cartid")), productImg, productName, productCode, price);
                        context.Response.Write(serializer.Serialize(new { Cart = GetCartDynamicObject(om.GetCart()), Success = true }));
                    }
                    else
                    {
                ...
                else if (action == "updateshippingaddress")
                {
                    ...
                    if (error.Count == 0)
                    {
                        om.UpdateOrderShippingAddress(int.Parse(CookieWorker.GetCookie(CookieWorker.OrderIdKey, "cartid")), shippingAddress,
                ...
                else if (action == "updatebillingaddress")
                {
                    ...
                    if (error.Count == 0)
                    {
                        om.UpdateOrderBillingAddress(int.Parse(CookieWorker.GetCookie(CookieWorker.OrderIdKey, "cartid")), billingAddress,
                ...
                else if (action == "updatecontact")
                {
                    ...
                    if (error.Count == 0)
                    {
                        om.UpdateOrderContact(int.Parse(CookieWorker.GetCookie(CookieWorker.OrderIdKey, "cartid")), name, email, memberid, phone);
                        context.Response.Write(serializer.Serialize(new { Success = true }));
                    }
                    else
                    {
