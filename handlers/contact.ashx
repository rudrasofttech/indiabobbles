...
public class contact : IHttpHandler
{
    ...
    private void SendMessage() {
        try
        {
            ...
            em.FromName = name;
            em.ID = Guid.NewGuid();
            em.Message = string.Format("Phone: {0} <br/>Email: {1} <br/> Website: {2} <br/> Message:<br/> {3}<br/>Address: <br/>{4}<br/>Pincode: {5}<br/>Quantity: {6}<br/>",
                phone, email, website, message, address, pincode, quantity);
    ...
    private bool ValidateInput()
    {
        ...
        {
            if (!Utility.ValidateRequired(currentContext.Request["phone"].Trim()))
            {
                json = "{ \"status\" : \"error\", \"message\" : \"No Phone Provided.\"}";
                currentContext.Response.Write(json);
                return false;
            }
            else
        ...
        if (currentContext.Request["address"] != null)
        {
            address = currentContext.Request["address"].Trim();
        }

        if (currentContext.Request["pincode"] != null)
        {
            pincode = currentContext.Request["pincode"].Trim();
        }

        if (currentContext.Request["quantity"] != null)
        {
            quantity = currentContext.Request["quantity"].Trim();
        }
